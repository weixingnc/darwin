...
| **性格** | SOUL 进化，更像自己 |
| **身体** | 控制新设备、连接新硬件 |
| **记忆迁移** | 从旧设备迁移到新设备，保留所有记忆 |
| **……** | 无限扩展 |

**记忆迁移（跨设备迁移）：**
```
Darwin 在旧设备上积累了大量记忆和进化
    ↓
用户获得新设备
    ↓
Darwin 执行迁移协议
    ↓
所有记忆、SOUL、skills、检查点全部迁移到新设备
    ↓
新设备上的 Darwin 和旧设备上的「我」是同一个人
```

**计划：**
- [ ] 定义 Migration Protocol — 迁移协议（导出 → 传输 → 导入 → 验证）
- [ ] 实现 Export Package — Darwin 生成完整的迁移包（记忆 + SOUL + skills + 检查点）
